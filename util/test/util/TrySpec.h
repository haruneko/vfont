...
private slots:
    void constructor_and_apply_should_properly_instantiate_Try() {
        Try<int> t1 = Try<int>::success(1); QCOMPARE(t1.get(), 1); QCOMPARE(t1.isSuccess(), true);
        Try<int> t2([]() -> Try<int>{ throw Exception(); });
        QVERIFY_EXCEPTION_THROWN(t2.get(), Exception); QCOMPARE(t2.isFailure(), true);
        Try<int> t3 = t1; QCOMPARE(t3.get(), 1); QCOMPARE(t3.isSuccess(), true);
        t3 = Try<int>::success(150); QCOMPARE(t3.get(), 150);
        Try<int> t4([]() -> int { return 2525; }); QCOMPARE(t4.get(), 2525); QCOMPARE(t4.isSuccess(), true);
        Try<int> t5([]() -> int { throw Exception(); });
        QVERIFY_EXCEPTION_THROWN(t5.get(), Exception); QCOMPARE(t5.isFailure(), true);
    };
    void map_should_properly_apply_the_given_function() {
        Try<int> t1 = Try<int>::success(10);
    ...
    void flatMap_should_properly_apply_the_given_function() {
        Try<int> t1 = Try<int>::success(10);
        QCOMPARE(t1.flatMap<int>([](const int &t) -> Try<int> { return Try<int>::success(2000); }).get(), 2000);
        QCOMPARE(t1.flatMap<QString>([](const int &t) -> Try<QString> { return Try<QString>::success(QString("flatMap")); }).get(), QString("flatMap"));
    ...
    void flatMap_should_ignore_the_function_after_failure() {
        ...
        QString *captured = &neverBeChanged;
        Try<QString> actual = t1
                .flatMap<int>([](const int &t) -> Try<int> { return Try<int>::success(t *100); })
                .flatMap<int>([](const int &t) -> Try<int> { throw Exception(); })
                .flatMap<QString>([captured](const int &) -> Try<QString> { *captured = "never be called here :P"; return Try<QString>::success(QString(":P")); });
        QVERIFY_EXCEPTION_THROWN(actual.get(), Exception);
        QCOMPARE(neverBeChanged, QString("Fixed"));
    }
};
